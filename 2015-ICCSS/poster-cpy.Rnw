...
\textbf{Code example}. Assume we are given simulated data matrix $X_{100 \times 300} \sim N(0, S)$, true signal $\beta$ and observed response variable $Y \sim N(X \beta, 1)$.
...
<<eval=TRUE, echo = FALSE, cache = FALSE, fig=TRUE, fig.height=2, fig.width=2>>=
base_size.gg <- 8
