...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\node [myboxwhite, right of=intro, node distance=40cm] (fdrill){%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\node [myboxwhite, below of=usageexample, node distance=5cm, yshift = -19cm] (usageexample2){%
  \begin{minipage}{0.48\textwidth}
  
\small
\textbf{FVR projected mean} Values euqal 0 indicate false discovery. 
<<eval=TRUE, echo = TRUE, cache = TRUE, fig=TRUE, fig.height=4, fig.width=5, fig.show='hold'>>=
# source("https://raw.githubusercontent.com/statsox/Penalized-Regression-Inference/master/R/PVR_proj_mean.R")

# Check how close to 0 ("false discovery") is actual true beta 
(beta.sel.tmp <- which(beta != 0))
FVR.proj.mean(beta.true = beta, beta.sel.idx = beta.sel.tmp, Sigma = Sigma)

# Make change in vector of "discoveries" we check for falses 
beta.sel.tmp <- c(c(7), beta.sel.tmp[-1])
FVR.proj.mean(beta.true = beta, beta.sel.idx = beta.sel.tmp, Sigma = Sigma)

# Make change in vector of "discoveries" we check for falses 
beta.sel.tmp <- c(c(1, 2), beta.sel.tmp[-1])
FVR.proj.mean(beta.true = beta, beta.sel.idx = beta.sel.tmp, Sigma = Sigma)
@


\textbf{$p-values$: Score test based on penalized regression} 
...
<<eval=TRUE, echo = FALSE, cache = FALSE, fig=TRUE, fig.height=2.5, fig.width=16, fig.show='hold'>>=
plot.discoveries(beta, beta.selected.idx)
@


\textbf{$p-values$: Multi sample-splitting} 
