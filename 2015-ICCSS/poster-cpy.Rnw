...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\node [myboxwhite, below of=usageexample, node distance=15cm, yshift = -19cm] (usageexample2){%
  \begin{minipage}{0.48\textwidth}
  
\textbf{\texttt{R}: FVP projected mean.} Values "close" to 0 indicate false discovery. 

\small
<<eval=TRUE, echo = TRUE, cache = TRUE, fig=TRUE, fig.height=4, fig.width=5, fig.show='hold'>>=
source("https://raw.githubusercontent.com/statsox/Penalized-Regression-Inference/master/R/false_variable_fw.R")

# How close to 0 ("false discovery") is actual true beta 
(beta.sel.idx <- which(beta != 0))
FVP.proj.mean(beta.true = beta, beta.sel.idx = beta.sel.idx, Sigma = Sigma)

# Make change in vector of "discoveries" we check for "false discovery" 
beta.sel.tmp <- c(c(7), beta.sel.tmp[-1])
round(FVP.proj.mean(beta.true = beta, beta.sel.idx = beta.sel.tmp, Sigma = Sigma),3)
@
