...
beta <- rep(0, p.tmp)
beta[sample(1:p.tmp, beta.sgnf.n, replace = FALSE)] <- 0.2
Y.true <- X %*% beta
Y <- scale(Y.true + matrix(rnorm(n.tmp)))

# Plots
vizu.mat(X, "X data (p>n, correlated predictors)", base_size = base_size.gg)
...
beta.is.selected <- which(res.lassoscore$p.model < 0.05)
