...
Assume we are given simulated data matrix $X_{100 \times 300} \sim N(0, S)$, true signa; $\beta$ and observed response variable $Y = X\beta + \varepsilon$, $\varepsilon \sim N(0, 1)$.
...
source("https://raw.githubusercontent.com/statsox/Penalized-Regression-Inference/master/R/Sigma_k_cor.R")
...
S <- Sigma.k.cor(0.4, p.tmp)
X <- scale(mvrnorm(n.tmp, mu = rep(0, p.tmp), Sigma = S))
beta <- rep(0, p.tmp)
beta[sample(1:p.tmp, beta.sgnf.n, replace = FALSE)] <- 10
Y.true <- X %*% beta
...
# lassoscore 
res.lassoscore <- lassoscore(Y, X, 0.2)
# res.lassoscore$fit$beta
sum(res.lassoscore$p.model < sgnf.lvl)
sum(res.lassoscore$p.model.cons < sgnf.lvl)
sum(res.lassoscore$p.sand < sgnf.lvl)
sum(res.lassoscore$p.sand.cons < sgnf.lvl)


# hdi 
## multisplit
res.hdi.multi <- hdi(X, Y, method = "multi.split",
                     model.selector = lasso.firstq,
                     args.model.selector = list(q = 10))
sum(res.hdi.multi$pval.corr < sgnf.lvl)
## stability
# res.hdi.stab <- hdi(X, Y, method = "stability", EV = ceiling(beta.sgnf.n * sgnf.lvl))
res.hdi.stab <- hdi(X, Y, method = "stability", EV = p.tmp * 0.5)
(res.hdi.stab$selected)


# Grace test 

# lambda.2 tuning parameter value for the ridge penalty

ridge.fit.cv <- cv.glmnet(X, Y, alpha = 0)
ridge.coef <- (coef(ridge.fit.cv))[-1]

# Compute coefficients by definition
ridge.fit.lambda <- ridge.fit.cv$lambda.1se * n.tmp
ridge.coef.DEF <- as.vector(solve(t(X) %*% X + ridge.fit.lambda * diag(p.tmp)) %*% t(X) %*% Y)

# Coefficients from Grace test
res.grace <- grace.test(Y, X, L = matrix(0, p.tmp, p.tmp), lambda.L = 0, lambda.2 = ridge.fit.lambda)
sum(res.grace$pvalue < sgnf.lvl)
```
...
# res.grace <- grace.test.C(Y, X, L = diag(p.tmp), lambda.L = ridge.fit.lambda, lambda.2 = 0,
#                           normalize.X  = FALSE, center.Y = FALSE)
# res.grace.coef <- res.grace$beta
